...
# ═══════════════════════════════════════════════════════════════
# SISTEMA PROMPT FINANCIERO
# ═══════════════════════════════════════════════════════════════


def _construir_contexto_financiero(resumen: dict, gastos_recientes: list) -> str:
    """Construye resumen financiero para enviar a la IA."""
    cats = resumen['categorias']
    lineas = [
        f"Mes: {resumen['mes']}/{resumen['anio']}",
        f"Total gastado: ${resumen['total_gastado']:,.0f} / ${resumen['total_presupuesto']:,.0f} ({resumen['porcentaje_global']}%)",
        "Por categoría:"
    ]
    for c in cats:
        lineas.append(f"  - {c['categoria']}: ${c['gastado']:,.0f} / ${c['limite']:,.0f} ({c['porcentaje_usado']}%)")
    
    if gastos_recientes:
        lineas.append(f"Últimos gastos: " + ", ".join(
            f"{g['descripcion']} ${g['monto']:,.0f}" for g in gastos_recientes[:5]
        ))
    
    return "\n".join(lineas)

def _calcular_proyeccion(resumen: dict) -> dict:
    """Proyecta el gasto total al fin de mes según ritmo actual."""
    hoy = date.today()
    dias_transcurridos = hoy.day
    dias_totales = 30  # aproximado
    ...
    for cat in resumen['categorias']:
        proyectado = cat['gastado'] * factor
        diferencia = cat['limite'] - proyectado
        proyecciones.append({
            'categoria': cat['categoria'],
            'gastado_actual': cat['gastado'],
            'proyectado_fin_mes': proyectado,
            'limite': cat['limite'],
            'diferencia': diferencia,
            'en_riesgo': proyectado > cat['limite']
        })
    
    total_proyectado = sum(p['proyectado_fin_mes'] for p in proyecciones)
...
with st.sidebar:
    ...
    nota_ingreso = st.text_input(
        "Nota", placeholder="Ej: Quincena + apoyo"
    )
...
with tab_resumen:
    ...
    if resumen['sin_ingreso']:
        st.warning("⚠️ No has registrado tu ingreso de este mes. Agrégalo en el sidebar izquierdo.")
    ...
    for i, (key, sobre) in enumerate(resumen['sobres'].items()):
        ...
        # ── Calcular todo ANTES del HTML ──────────────────────
        if not resumen['sin_ingreso']:
            if sobre['sobre_lleno']:
                estado_txt   = "✅ Sobre completo"
                estado_color = '#3fb950'
            elif sobre['presupuesto'] == 0:
                estado_txt   = "⚠️ Sin fondos este mes"
                estado_color = '#f85149'
            else:
                estado_txt   = "○ Sobre parcial"
                estado_color = '#e3b341'
        else:
            estado_txt   = "— Sin ingreso"
            estado_color = '#8b949e'
        
        # Calcular color disponible ANTES del HTML
        color_disp     = '#3fb950' if sobre['disponible'] >= 0 else '#f85149'
        pct_int        = int(sobre['pct'] * 100)
        pct_barra      = min(pct_s, 100)
        gastado_fmt    = f"${sobre['gastado']:,.0f}"
        presup_fmt     = f"${sobre['presupuesto']:,.0f}"
        disp_fmt       = f"${sobre['disponible']:,.0f}"
        cantidad       = sobre['cantidad_gastos']
        nombre         = sobre['nombre']
        descripcion_s  = sobre['descripcion']
        emoji_s        = sobre['emoji']
...
<div class="sobre-card {css}">
    ...
    <div style="color:#8b949e; font-size:0.75rem; margin-bottom:0.75rem;">
        {descripcion_s} • {pct_int}%
    </div>
    <div style="background:#21262d; border-radius:4px; height:8px; margin-bottom:0.75rem;">
        <div style="background:{color_b}; width:{pct_barra:.0f}%;
                    height:100%; border-radius:4px;"></div>
    </div>
    <div style="display:flex; justify-content:space-between; font-size:0.85rem;">
        <span style="color:#f0f6fc; font-weight:600;">{gastado_fmt}</span>
        <span style="color:{color_b}; font-weight:600;">{pct_s:.0f}%</span>
    </div>
    <div style="display:flex; justify-content:space-between;
                font-size:0.75rem; margin-top:0.2rem;">
        <span style="color:#8b949e;">de {presup_fmt}</span>
        <span style="color:{color_disp};">Disp: {disp_fmt}</span>
    </div>
    ...
    # ── Lógica de llenado visual ──────────────────────────────
    if not resumen['sin_ingreso']:
        st.divider()
        st.markdown("### 📋 Orden de llenado")
        st.caption(
            "Si el ingreso es menor a lo esperado, los sobres se llenan en este orden:"
        )
        
        # ── CORRECCIÓN: crear columnas ANTES del loop ─────────
        ...
        for i, (key, sobre) in enumerate(resumen['sobres'].items()):
            ...
            bg_color     = '#0f2d0f' if lleno else '#1c2128'
            border_color = sobre['color'] if lleno else '#30363d'
            txt_color    = '#3fb950' if lleno else '#e3b341'
            nombre_corto = sobre['nombre'][:14]
            emoji_s      = sobre['emoji']
            
            if lleno:
                estado_ord = '✅ Completo'
            else:
                estado_ord = f"${asignado:,.0f} / ${presup_ideal:,.0f}"
            
            # ── Usar cols_ord[i] directamente ─────────────────
            with cols_ord[i]:
                st.markdown(f"""
<div style="text-align:center; padding:0.75rem;
            ...
            align-items:center;">
    <div style="font-size:1.8rem; margin-bottom:0.25rem;">{emoji_s}</div>
    <div style="color:#f0f6fc; font-size:0.8rem; font-weight:600;">
        {i+1}. {nombre_corto}
    </div>
    <div style="color:{txt_color}; font-size:0.75rem; margin-top:0.4rem; font-weight:600;">
        {estado_ord}
    </div>
</div>
                """, unsafe_allow_html=True)
    ...
    with st.form("form_nuevo_gasto", clear_on_submit=True):
        ...
        with col_sobre:
            sobre_sel = st.selectbox(
                ...
                format_func=lambda x: (
                    f"{SOBRES_CONFIG[x]['emoji']} "
                    f"{SOBRES_CONFIG[x]['nombre']}"
                )
            )
        
        # Subcategorías dinámicas según sobre
        ...
        # Mostrar si es gasto fijo (solo Sobre 1)
        es_fijo = False
        ...
        submitted = st.form_submit_button(
            "💾 Guardar Gasto", use_container_width=True, type="primary"
        )
    ...
    if not gastos_h:
        ...
        for g in gastos_h:
            ...
            with col1:
                ...
                st.markdown(
                    f"**{emoji} {g['descripcion']}{fijo_tag}**  \n"
            ...
            with col3:
                st.markdown(
                    ...
                    f"font-size:0.7rem;'>{emoji}</span></div>",
                    unsafe_allow_html=True
                )
            st.divider()
...
with tab_ia:
    ...
    def _contexto_ia(r: dict) -> str:
        ...
        for key, s in r['sobres'].items():
            # Presupuesto asignado según % del ingreso
            ...
            lineas.append(
                f"  {s['emoji']} {s['nombre']}:"
                f"\n     - Presupuesto asignado: ${presup:,.0f}"
                f"\n     - Gastado hasta hoy:    ${gastado:,.0f}"
                f"\n     - Disponible:           ${disp:,.0f}"
                f"\n     - Uso:                  {pct:.0f}%"
            )
        ...
        lineas += [
            "",
            "IMPORTANTE: Si el gasto es 0% en un sobre, significa que",
            "el dinero está disponible pero aún no se ha usado, NO que",
            "el sobre esté vacío. El ingreso ya fue registrado.",
        ]
        
        return "\n".join(lineas)
    
    ctx = _contexto_ia(resumen)
    ...
    if st.button("🤖 Analizar proyección", key="btn_proy"):
        ...
        dias_hoy = date.today().day
        
        prompt = f"""
    Analiza la situación financiera de este mes:

    {_contexto_ia(resumen)}

    Proyección matemática (basada en {dias_hoy} días transcurridos):
    """
    ...
    Sobres en riesgo de exceder: {riesgo or 'Ninguno'}

    Si el gasto es $0 en algún sobre, NO digas que no hay gastos como problema.
    Significa que el dinero está disponible y no se ha gastado aún — eso es positivo.
    Da 2 observaciones reales y 1 acción concreta para los próximos días.
    """
    ...
    criticos = [
        s for s in resumen['sobres'].values()
        if s['pct_usado'] >= 80
    ]
    ...
    if pregunta:
        ultimos = ", ".join(
            f"{g['descripcion']} ${g['monto']:,.0f}"
            for g in gastos_ia[:5]
        )
        with st.spinner("Pensando..."):
            st.info(chat_simple(
...
with tab_editar:
    ...
    with col_b:
        ...
        if st.button("🔍 Buscar", use_container_width=True):
            ...
            encontrado = next(
                (g for g in todos if g['id'] == id_buscar), None
            )
            if encontrado:
                st.session_state.gasto_encontrado = encontrado
                st.success(
                    f"✅ {encontrado['descripcion']} · "
                    f"${encontrado['monto']:,.2f}"
                )
            else:
        ...
        with st.form("form_editar"):
            ...
            with col_n:
                nuevas_notas = st.text_input(
                    "Notas", value=gasto.get('notas') or ""
                )
