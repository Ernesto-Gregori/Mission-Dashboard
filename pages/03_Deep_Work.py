...
from app.database import init_database, DB_PATH, obtener_tipos_bloque
...
with tab_config:
    ...
    def crear_bloque(nombre, hora_inicio, hora_fin, dias, tipo, color):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
            cursor.execute("""
    ...
    def actualizar_bloque(bloque_id, nombre, hora_inicio, hora_fin, dias, tipo, color, activo):
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        try:
            cursor.execute("""
    ...
    def obtener_todos_bloques():
        """Incluye inactivos para gestión completa"""
        conn = sqlite3.connect(DB_PATH)
        conn.row_factory = sqlite3.Row
        cursor = conn.cursor()
        cursor.execute("""
    ...
    if st.session_state.mostrar_form_nuevo:
        ...
        with st.form("form_nuevo_bloque", clear_on_submit=True):
            nuevo_nombre = st.text_input(
                "Nombre del bloque *",
                placeholder="Ej: Deep Work: Código"
            )

            col_t, col_t_nuevo = st.columns([1, 1])
            with col_t:
            ...
            es_nuevo_tipo = nuevo_tipo_sel == "✏️ Nuevo tipo..."

            with col_t_nuevo:
                nuevo_tipo_custom = st.text_input(
                    "Nuevo tipo (si elegiste Escribir nuevo)",
                    placeholder="Ej: Trabajo, Estudios, Empleo..."
                )

            # Resolver tipo final
            nuevo_tipo = nuevo_tipo_custom.strip() if es_nuevo_tipo else nuevo_tipo_sel

            col_hi, col_hf, col_col = st.columns(3)
            with col_hi:
                nuevo_inicio = st.text_input(
                    "Hora inicio *", placeholder="06:15",
                    help="Formato HH:MM"
                )
            with col_hf:
                nuevo_fin = st.text_input(
                    "Hora fin *", placeholder="07:15",
                    help="Formato HH:MM"
                )
            with col_col:
            ...
            if submit_nuevo:
                ...
                if nuevo_tipo_sel == "✏️ Escribir nuevo..." and not nuevo_tipo_custom.strip():
    ...
    for b in todos:
        ...
        col_card, col_editar, col_eliminar = st.columns([5, 1, 1])

        with col_card:
            st.markdown(f"""
<div class="bloque-card" style="opacity:{opacidad};">
    <div style="display:flex; justify-content:space-between; align-items:center;">
        <span style="color:{color_b}; font-weight:700; font-size:1rem;">
            {b['nombre']}
        </span>
    ...
    <div style="color:#8b949e; font-size:0.8rem; margin-top:0.4rem;">
        ...
        with col_editar:
            if st.button("✏️", key=f"edit_{b['id']}", help="Editar bloque",
                         use_container_width=True):
        ...
        with col_eliminar:
            if st.button(
                "🗑️" if activo else "♻️",
                key=f"del_{b['id']}",
                help="Desactivar bloque" if activo else "Reactivar bloque",
                use_container_width=True
            ):
                if activo:
                    eliminar_bloque(b['id'])
        ...
        if st.session_state.bloque_editando == b['id']:
            with st.form(f"form_editar_{b['id']}"):
                ...
                col_et, col_et_nuevo = st.columns([1, 1])
                with col_et:
                    edit_tipo_sel = st.selectbox(
                        "Tipo",
                        options=TIPOS_BLOQUE + ["✏️ Nuevo tipo..."],
                        index=TIPOS_BLOQUE.index(b['tipo']) if tipo_en_lista else len(TIPOS_BLOQUE)
                    )

                es_nuevo_tipo_edit = edit_tipo_sel == "✏️ Nuevo tipo..."
                ...
                # Resolver tipo final
                edit_tipo = edit_tipo_custom.strip() if es_nuevo_tipo_edit else edit_tipo_sel
                ...
                with col_ecol:
                    # Encontrar el label del color actual
                    color_actual_label = next(
                        (k for k, v in COLORES.items() if v == b['color']),
                        list(COLORES.keys())[0]
                    )
                    edit_color_label = st.selectbox(
                        "Color", list(COLORES.keys()),
                        index=list(COLORES.keys()).index(color_actual_label)
                    )
                ...
                if submit_edit:
                    ...
                    if edit_tipo_sel == "✏️ Escribir nuevo..." and not edit_tipo_custom.strip():
