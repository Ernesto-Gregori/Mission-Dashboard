...
def actualizar_idea(idea_id: int, titulo: str, descripcion: str,
    ...
    """, [titulo, descripcion, dominio, categoria,
          ...
          motivacion, notas, datetime.now().isoformat(), idea_id])


def eliminar_idea(idea_id: int) -> None:
    ejecutar("DELETE FROM sandbox_ideas WHERE id=?", [idea_id])
...
def eliminar_snippet(snip_id: int) -> None:
    ejecutar("DELETE FROM sandbox_snippets WHERE id=?", [snip_id])
...
with tab_ideas:
    ...
    with col_f3:
        f_busqueda = st.text_input("Buscar",  placeholder="Título, descripción...", key="f_bus_ideas")
    with col_f4:
    ...
    if st.session_state.mostrar_form_idea:
        ...
        with col_cat_new:
            n_cat_custom = st.text_input(
                "Nueva categoría", placeholder="Ej: Vacaciones", key="n_cat_custom"
            )
        ...
        with st.form("form_nueva_idea", clear_on_submit=True):
            ...
            with col_n1:
                n_titulo = st.text_input("Título *", placeholder="Ej: Salida familiar al lago")
            with col_n2:
                n_estado = st.selectbox("Estado inicial", ESTADOS_IDEA)

            n_desc = st.text_area("Descripción",
                placeholder="¿Qué es esta idea? ¿Por qué te interesa?", height=80)

            col_n3, col_n4, col_n5 = st.columns(3)
            with col_n3:
                n_prioridad = st.select_slider("Prioridad", options=[1,2,3,4,5], value=3,
            ...
            with col_sc:
                cancel_idea = st.form_submit_button("✖ Cancelar", use_container_width=True)
...
<div style="background:#161b22;border:1px solid #30363d;
    ...
    {f'<div style="color:#8b949e;font-size:0.75rem;margin-top:0.3rem;">📝 {notas_txt[:60]}{"..." if len(notas_txt)>60 else ""}</div>' if notas_txt else ''}
            ...
            if st.session_state.get(f"del_idea_{idea['id']}"):
                ...
                with c2:
                    if st.button("🗑️ Sí", key=f"cdi_{idea['id']}", use_container_width=True):
                        eliminar_idea(idea["id"])
                        st.session_state[f"del_idea_{idea['id']}"] = False
                        st.rerun()
                with c3:
                    if st.button("✖", key=f"cni_{idea['id']}", use_container_width=True):
                        st.session_state[f"del_idea_{idea['id']}"] = False
                        st.rerun()
            ...
            if st.session_state.idea_editando == idea["id"]:
                ...
                with col_ec:
                    ecat_sel = st.selectbox(
                        ...
                        index=cats_edit.index(cat_actual) if cat_actual in cats_edit else 0,
                ...
                with st.form(f"form_edit_idea_{idea['id']}"):
                    ...
                    with col_e2:
                        eest = st.selectbox("Estado", ESTADOS_IDEA,
                    ...
                    with col_e3:
                        epri = st.select_slider("Prioridad", options=[1,2,3,4,5],
                            value=idea.get("prioridad",3),
                            format_func=lambda x: {
                                1:"⚪ Baja",2:"🔵 Normal",3:"🟡 Media",
                                4:"🟠 Alta",5:"🔴 Urgente"}[x])
                    with col_e4:
                        emot = st.slider("Motivación", 1, 10, value=idea.get("motivacion",7))
                    with col_e5:
                        etags_str = st.text_input("Etiquetas", value=", ".join(etiq_act))
                        ...
                        if st.form_submit_button("💾 Guardar",
                            ...
                            if not et.strip():
                                ...
                                actualizar_idea(
                                    ...
                                    [t.strip() for t in etags_str.split(",") if t.strip()],
...
with tab_snippets:
    ...
    if st.session_state.mostrar_form_snip:
        ...
        with st.form("form_nuevo_snip", clear_on_submit=True):
            ...
            with col_sc2:
                cancel_snip = st.form_submit_button("✖ Cancelar", use_container_width=True)
    ...
    if not snippets:
        ...
        for s in snippets:
                ...
                with st.expander(
                    ...
                    with col_uso:
                        if st.button("✅ Marcar como usado",
                                     key=f"uso_{s['id']}", use_container_width=True):
            ...
            if st.session_state.get(f"del_snip_{s['id']}"):
                ...
                with c2:
                    if st.button("🗑️ Sí", key=f"cds_{s['id']}", use_container_width=True):
                        eliminar_snippet(s["id"])
                        st.session_state[f"del_snip_{s['id']}"] = False
                        st.rerun()
                with c3:
                    if st.button("✖", key=f"cns_{s['id']}", use_container_width=True):
                        st.session_state[f"del_snip_{s['id']}"] = False
                        st.rerun()
            ...
            if st.session_state.snip_editando == s["id"]:
                ...
                with st.form(f"form_edit_snip_{s['id']}"):
                    ...
                    with col_se1:
                        ...
                        sel   = st.selectbox("Lenguaje", LENGUAJES,
                            index=LENGUAJES.index(s["lenguaje"])
                            if s["lenguaje"] in LENGUAJES else 0)
                    with col_se2:
                        sed   = st.selectbox("Dominio", DOMINIOS,
                            index=DOMINIOS.index(s.get("dominio","Personal"))
                            if s.get("dominio") in DOMINIOS else 0)
                        set_tags = st.text_input("Tags", value=", ".join(tags_act))

                    sec  = st.text_area("Código", value=s.get("codigo",""), height=180)
...
with tab_sesion:
    ...
    with col_form:
        ...
        fecha_s    = st.date_input("Fecha", value=date.today(), key="fecha_sesion")
        dom_sesion = st.selectbox("Dominio de la sesión", DOMINIOS, key="dom_ses")
        duracion_s = st.slider("Duración (min)", 15, 240, 60, step=15, key="dur_ses")
        ...
        proy_id = st.selectbox(
            ...
            format_func=lambda x: next(p[1] for p in opciones_proy if p[0] == x),
            key="proy_ses"
        )
        desc_s   = st.text_area("¿Qué hiciste?", height=100,
            placeholder="Describe lo que exploraste, aprendiste o avanzaste...",
            key="desc_ses")
        codigo_s = st.text_area("Código / notas producidas (opcional)", height=100,
        ...
        if st.button("💾 Guardar sesión", use_container_width=True,
            ...
            if not desc_s.strip():
                ...
                guardar_sesion(fecha_s, duracion_s, tipo_act, dom_sesion,
...
with tab_ia:
    ...
    with col_ctx:
        ...
        if ideas_dom:
            usar_idea = st.checkbox("Consultar sobre una idea específica",
                                    key="cb_usar_idea")
            if usar_idea:
        ...
        tipos_ayuda = {
            "Programacion": ["Planificar pasos del proyecto","Sugerir librerías / herramientas",
                             "Revisar enfoque técnico","Ayuda para depurar","Inspiración para nuevo proyecto"],
            "Estudio":      ["Plan de estudio para este tema","Recursos y bibliografía",
                             "Conexión con otros temas","Preguntas de comprensión","Resumen del concepto"],
            "Personal":     ["Plan de acción para esta meta","Hábitos relacionados",
                             "Obstáculos comunes y cómo superarlos","Reflexión bíblica sobre el tema","Próximo paso concreto"],
            "Familia":      ["Ideas para fortalecer esta área","Conversaciones importantes",
                             "Actividades juntos","Reflexión cristiana","Cómo priorizar esto"],
            "Ministerio":   ["Cómo desarrollar este proyecto","Recursos bíblicos relevantes",
                             "Plan de implementación","Cómo involucrar a otros","Reflexión sobre el impacto"],
        }
        opciones_ayuda = tipos_ayuda.get(dominio_ia, [
            "Plan de acción","Recursos útiles","Próximo paso",
            "Reflexión bíblica","Cómo priorizar",
        ])
        tipo_ayuda_ia = st.selectbox("Tipo de ayuda", opciones_ayuda, key="tipo_ayuda_ia")
        ...
        if ideas_total:
            ...
            activas_dom = [i for i in ideas_total
                           if i["estado"] not in ["Completado","Abandonado"]]
            st.caption(f"📊 {len(ideas_total)} ideas en {dominio_ia} · 🔄 {len(activas_dom)} activas")
